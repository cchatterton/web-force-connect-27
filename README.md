...
WFC27 is a one-minute, bidirectional JSON transport between Salesforce and WordPress. It installs a durable station on each side and shows every train trip, including empty trains. It does not inspect the JSON or process business records. Data processing engines are separate future packages.

## The stations

The Salesforce unmanaged package adds `WFC27_Station__c` with an envelope ID, JSON payload, and status. The WordPress plugin creates the `wfc27_station` table with the same three fields. The local platform's standard record ID and timestamps remain available for administration. The envelope ID is unique across a station; repeated train delivery does not duplicate a staged item.

Statuses are `outbound_ready`, `outbound_delivered`, `inbound_received`, and `inbound_acked`. An engine can later place an outbound JSON object into the local station and claim inbound JSON. WFC27 never updates posts or Salesforce business objects. A train records transport receipt; it is not a claim that a future engine processed the JSON.
...
The on-wire protocol is `wfc27.station.v1`:

```json
{
  "protocol": "wfc27.station.v1",
  "envelopes": [{"id": "stable-envelope-id", "payload": {"any": "JSON object"}}],
